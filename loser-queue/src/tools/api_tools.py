    ...
    if tier in ["CHALLENGER", "GRANDMASTER", "MASTER"]:
        ...
        for division in divisions:
            ...
            while len(summoner_names_division) < number_by_division:
                entries = get_active_entry_by_ranking(
...
def get_last_matches_of_summoner_by_puuid(
    summoner_puuid: str, number_of_matches: int, max_match_id=None
):
    if max_match_id:
...
def get_last_matches_of_summoner_by_summoner_name(
    summoner_name: str, number_of_matches: int, max_match_id=None
):
    try:
